...
| `media.py` | Not a stage. `materialize_media` pulls the figures the HTML references (`media/<sha256>.<ext>`) out of CAS for the print renderers (`paginate`, `paginate-typst`), passing each through `opaque`, which composites any alpha onto white. Print has no transparency, and one alpha figure made Ghostscript rasterize a whole book at PDF/X-1a. A referenced blob missing from CAS raises. |
| `rendering.py` | Not a stage. `ast_to_html` (print; `extract` and `paginate` must call the SAME object), `ast_to_epub_sections` (EPUB), `emit_css`. Print footnotes: `PrintNotes` numbers them itself -- an empty `note-call` span drawn by CSS `attr(data-n)`, `data-n` on the note for its marker -- because weasyprint's call belongs to the paragraph and cannot be restyled per note. A note over `NOTE_SPLIT_CHARS` is set in sentence-end pieces (`footnote-cont`, no call or marker) weasyprint can carry over, and the `@footnote` area is capped at `NOTE_AREA_MAX`: page-long notes squeezed pages to one body line (orphans weasyprint cannot avoid). The Typst Lua filter folds the pieces back into one `#footnote`. Print only: `_keep_tail` wraps the last two words of every paragraph, heading, chapter title and note piece in `<span class="keep">` (`white-space: nowrap`) so no last line holds one word. The pair may span text runs; a paragraph's note calls go inside it; past `KEEP_TAIL_CHARS` (40; `KEEP_CELL_CHARS` 16 in table cells) the last 3/4-cap characters are kept instead (URLs). Same characters, so the integrity gate is unaffected -- tests that look for a whole sentence in the HTML must strip tags first. `paginate`'s runt = a one-word last line under `RUNT_MAX_FILL` of its own block's width. |
...
| `paginate_stage.py` | `paginate` · `doc-effective/1 + text/css → raw-pdf/1` (+ terminal `pagemap/1`). Neither input is root — that is the DAG bypass fix. `notes_in_document_order()` (called before every render, idempotent) hooks weasyprint 70's `make_page` and `LayoutContext._update_footnote_area` to keep carried-over footnotes in `data-seq` order — 70 reverses notes it moves for orphans/widows. Do **not** reach for `footnote-policy: line` instead: it stranded call lines (27 widows, 23 one-line pages); `block` crashes 70. |
