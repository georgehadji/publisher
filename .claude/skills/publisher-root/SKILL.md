...
| `docker-compose.yml` | `postgres` + `migrate` + `worker` + `api` + `llm-egress-proxy` (E6.2). **E3.1/E6.2:** `worker` sits on `dbnet` (`internal: true` — no route to the internet) and `llm-egress` (ALSO `internal: true` — joining it alone grants nothing). `llm-egress-proxy` (built from `infra/llm-egress/`, a tinyproxy allowlisting only `openrouter.ai`) is the one thing on BOTH `llm-egress` and `llm-egress-uplink` (the only non-internal network in the file) — that is what makes it a real egress gateway rather than just another `llm-egress` member with its own back-door route out. `OPENROUTER_API_KEY` is unset by default, which is what keeps `structure-infer` unreachable; `HTTPS_PROXY`/`HTTP_PROXY` point `worker` at the proxy so `OpenRouterProvider`'s `urllib` calls actually go through the allowlist. **Postgres publishes 55432, not 5432** — a locally installed Postgres commonly owns 5432, and when it does, the host-side integration tests silently connect to *that* database instead. **E4.1:** `migrate` (built from `Dockerfile.api`, runs `packages/api/dist/migrate.js`) applies `platform/db/migrations/*.sql` before `worker`/`api` start (`depends_on: condition: service_completed_successfully`) — the old `docker-entrypoint-initdb.d` schema.sql mount only ever ran against an empty data directory. **E4.3:** `worker`/`api` connect as the unprivileged `publisher_worker`/`publisher_app` roles, not the owner/superuser `publisher` (which `migrate` still uses); `api` also gets `PUBLISHER_ADMIN_DATABASE_URL` (the `publisher_admin` BYPASSRLS role) for `/v1/admin/metrics`'s cross-tenant aggregates. Both worker and api mount the shared `cas-data` volume **read-write** (the API writes uploads). `PYTHONUNBUFFERED=1` on the worker, or `docker compose logs worker` stays empty for a whole build. |
...
| `requirements.txt` | The only place third-party Python deps are actually pinned (every package's `pyproject.toml` declares `dependencies = []` by pre-existing convention). Notable pins: `weasyprint>=70,<71` (70 cleared 62.3's three advisories; the old `pydyf<0.11` cap was 62.x-only and is gone — `requirements.lock` carries pydyf 0.12.1; on Windows 70 finds GTK only through `WEASYPRINT_DLL_DIRECTORIES`, which `scripts/test.ps1` sets from PATH), `psycopg2-binary`, `PyYAML`, `jsonschema`, `python-docx` (**legacy binary `.doc` is not supported — convert with LibreOffice first**). |
...
| `.gitignore` | Note the non-obvious groups: `*.gen.*` — its comment claims "checked in", which is **false**: they are ignored, zero are tracked, and `gen:check` therefore cannot fail (see **publisher-schemas**); `.publisher/` / `.test-cas-cache/` / `.cas-cache/` (durable CAS blobs + sqlite cache index; content-addressed output, not source); and `package-lock.json` with two explicit `!packages/api/...` / `!packages/web/...` negations (E5.3-era CI fix) — both are npm-managed independently of the pnpm workspace despite matching `pnpm-workspace.yaml`'s `packages/*` glob, and `npm ci` in `.github/workflows/ci.yml` needs a committed lockfile to run at all. |
| `.dockerignore` (E6.1) | Excludes every `tests/` directory from **every** Docker build context (`Dockerfile.api`, `Dockerfile.worker`, `infra/llm-egress/`). Not hygiene — `Dockerfile.worker` puts each service's ROOT on `PYTHONPATH` (e.g. `/app/services/structure`), so without this, Python 3's implicit namespace packages would make a `tests/` sibling of `publisher_structure/` importable inside the worker container despite never being `pip install`ed. This is what actually keeps `services/structure/tests/fabricating_provider.py` out of production, not just its file location. |
